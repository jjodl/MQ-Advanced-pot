...
1. Copy all key.* files to acemq2 Virtual Machines using sftp.   Login to acemq2 using password *engageibm* <br>
   ```
   sftp ibmuser@acemq2
   ```
   ``` 
...
1. On acemq2, acemq3 run the following commands. <br>
   ```
   sudo chown -R :mqm /var/mqm/qmgrs/MQ01HA/ssl/key.*
   ```
   ```
...
1. On each VM (acemq1,2,3), append the TLS parameters as well as all 3 NativeHAInstances to qm.ini. <br>
You can chose either to edit with **vi** or **nano** which ever one you are more familiar with.   **nano** is easies to use. 
   ```
   vi /var/mqm/qmgrs/MQ01HA/qm.ini
   ```
      *or*
   ```
   nano /var/mqm/qmgrs/MQ01HA/qm.ini
   ```
   ![alt text](images/qm1.png)

1. Add the following section to the end of the **qm.ini** file.
...
NativeHAInstance:
   Name=acemq1
   ReplicationAddress=acemq1(9414)
NativeHAInstance:
   Name=acemq2
   ReplicationAddress=acemq2(9414)
NativeHAInstance:
   Name=acemq3
   ReplicationAddress=acemq3(9414)
```
3. When done should look simliar to this. 
